...
<template>
    <div class="flex items-center space-x-3 group select-none">
        ...
        <div class="relative flex-shrink-0" :class="iconSize">
            ...
            <!-- Logo Box -->
            <div class="relative w-full h-full rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 p-1.5 flex items-center justify-center shadow-md overflow-hidden">
                <img 
                    :src="logoUrl" 
        ...
        <div v-if="variant === 'full'" class="flex flex-col">
            <div class="flex items-center space-x-2">
                <span 
                    class="font-black tracking-tight leading-none bg-gradient-to-r from-orange-600 via-amber-600 to-orange-500 dark:from-orange-400 dark:via-amber-300 dark:to-orange-400 bg-clip-text text-transparent"
                    :class="textSize"
                >
                    MKT INDONESIA
                </span>
                <span class="inline-block w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Sistem Siaga Bencana Aktif 24/7"></span>
            </div>
            <span 
                v-if="showSubtitle" 
                class="text-[9px] sm:text-[10px] font-bold tracking-wider text-slate-500 dark:text-slate-400 uppercase mt-0.5"
            >
